...
export const mainMenuTemplate = async (
  ...
  return [
    ...
    {
      label: 'About AuraMusic',
      submenu: [
        {
          label: 'About AuraMusic Desktop',
          click() {
            dialog
              .showMessageBox(win, {
                type: 'info',
                title: 'About AuraMusic Desktop',
                message: 'AuraMusic Desktop v1.0.0',
                detail:
                  'Created & Maintained by iammrwrath\n\n' +
                  'Elevated YouTube Music desktop experience with Windows 11 Fluent styling, live lyrics translation, real-time karaoke vocal suppression, and OBS broadcasting.\n\n' +
                  'Special Acknowledgments & Credits:\n' +
                  '• Metrolist & BitChord (Framework & lyrics core)\n' +
                  '• CodeMathis & th-ch (YouTube Music Desktop Electron)\n\n' +
                  'License: MIT License',
                buttons: ['Close', 'GitHub Repository', 'AuraMusic Android'],
                defaultId: 0,
                cancelId: 0,
              })
              .then(({ response }) => {
                if (response === 1) {
                  shell.openExternal('https://github.com/iammrwrath/AuraMusic-Desktop');
                } else if (response === 2) {
                  shell.openExternal('https://github.com/iammrwrath/AuraMusic');
                }
              });
          },
        },
        {
          label: 'Visit AuraMusic GitHub',
          click() {
            shell.openExternal('https://github.com/iammrwrath/AuraMusic-Desktop');
          },
        },
        {
          label: 'AuraMusic Android App',
          click() {
            shell.openExternal('https://github.com/iammrwrath/AuraMusic');
          },
        },
